    ...
    if (isset($_POST['add_opp'])) {
        ...
        if (empty($title) || empty($description) || empty($category) || empty($link)) {
            ...
            try {
                ...
                $stmt->execute([':title'=>$title, ':description'=>$description, ':category'=>$category, ':region'=>$region, ':created_by'=>$created_by_admin_id, ':link'=>$link]);
                $message = '<div class="alert alert-success">Opportunité "<b>' . htmlspecialchars($title) . '</b>" publiée avec succès!</div>';
            } catch (PDOException $e) {
                $message = '<div class="alert alert-danger">Erreur : ' . $e->getMessage() . '</div>';
            }
        }
    } elseif (isset($_POST['delete_opp'])) {
        $id = $_POST['id'];
        try {
            $sql = "DELETE FROM opportunities WHERE opp_id = :id";
            $stmt = $conn->prepare($sql);
            $stmt->execute([':id' => $id]);
            $message = '<div class="alert alert-success">Opportunité supprimée avec succès.</div>';
        } catch (PDOException $e) {
            $message = '<div class="alert alert-danger">Erreur : ' . $e->getMessage() . '</div>';
        }
    }
...
$opportunities = [];
try {
    $sql_select = "SELECT o.*, a.username AS admin_username FROM opportunities o JOIN admins a ON o.created_by = a.admin_id ORDER BY o.created_at DESC";
...
<head>
    ...
    <title>Opportunités - Nafas Admin</title>
...
<body>
    ...
    <ul>
        ...
        <li class="hovered"><a href="gestion_opportunites.php"><span class="icon"><ion-icon name="briefcase-outline"></ion-icon></span><span class="title">Opportunités</span></a></li>
        ...
        <div class="main">
            ...
            <div class="details">
                ...
                <div class="recentOrders mb-4">
                    <div class="cardHeader"><h2>Ajouter une Opportunité</h2></div>
                    <form action="gestion_opportunites.php" method="POST">
                ...
                <div class="recentOrders">
                    <div class="cardHeader"><h2>Liste des Opportunités</h2></div>
                            ...
                            <?php if (count($opportunities) > 0): ?>
                                <?php foreach ($opportunities as $opp): ?>
                                    <tr>
                                        ...
                                        <td><span class="badge bg-secondary"><?php echo ucfirst($opp['category']); ?></span></td>
                                        <td><?php echo date('d/m/Y', strtotime($opp['created_at'])); ?></td>
                                        <td><?php echo htmlspecialchars($opp['admin_username']); ?></td>
                                        <td>
                                            <a href="<?php echo htmlspecialchars($opp['link']); ?>" target="_blank" class="status delivered" style="text-decoration: none;">Voir</a>
                                            <form action="gestion_opportunites.php" method="POST" style="display:inline;" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette opportunité ?');">
                                                <input type="hidden" name="delete_opp" value="1">
                                                <input type="hidden" name="id" value="<?php echo $opp['opp_id']; ?>">
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr><td colspan="5" class="text-center">Aucune opportunité.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
